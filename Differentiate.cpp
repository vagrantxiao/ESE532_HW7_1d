...
#pragma SDS data copy(Input[0:INPUT_FRAME_WIDTH*2], Output[0:INPUT_FRAME_WIDTH])
#pragma SDS data access_pattern(Input:SEQUENTIAL, Output:SEQUENTIAL)
void Differentiate_HW(const unsigned char Input[INPUT_FRAME_SIZE], unsigned char Output[INPUT_FRAME_SIZE])
{
  for (int Y = 0; Y < OUTPUT_FRAME_HEIGHT; Y++)
    for (int X = 0; X < OUTPUT_FRAME_WIDTH; X++)
    {
